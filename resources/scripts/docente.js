...
function Save () {
	...
	if($('#fname').val().length < 1) {  
		...
		var nombre = $('#fname').val()+' '+$('#lname').val(),
			nombreArray = nombre
						.normalize('NFD')
						...
						.toLowerCase()
						.split(' ');
			username = nombreArray[0]+'.'+nombreArray[2]+'LI';

		$.ajax({
			url: url+'Persona/savePersona',
			type: 'post',
			data: $('#formulario').serialize()+'&username='+username+'&rol=2',
			beforeSend: (function(){
				$('#button').html(
...
function Edit (id) {
	...
	if($('#fname').val().length < 1) {  
		...
		$.ajax({
			...
			type: 'post',
			data: $('#formulario').serialize()+'&id='+id+'&rol=2',
